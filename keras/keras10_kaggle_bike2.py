# abs()로 y_submit을 감싸면 결과가 나오는지 궁금했음.
# 결론 : 안나옴..
...
start_time = tm.time()
model.fit(x_train, y_train, epochs = 1000, batch_size = 381, verbose = 2)
end_time = tm.time()
run_time = round(end_time - start_time, 2)

#4. 평가 예측
loss = model.evaluate(x_test, y_test)
y_submit = abs(model.predict(test_csv))     # 그냥 전부 절대값 씌운거
y_predict = model.predict(x_test)
r2 = r2_score(y_test, y_predict)

submission_csv['count'] = y_submit

submission_csv.to_csv(path + "submission_0108_2", index = False)

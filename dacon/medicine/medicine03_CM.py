...
train_x, val_x, train_y, val_y = train_test_split(train_x, train_y, test_size=0.18, random_state=42)
...
#2
model = Sequential()
model.add(Dense(64, input_shape=(2048,)))
...
es = EarlyStopping(monitor='mse', mode='min', verbose=1, patience=100, restore_best_weights=True)

hist = model.fit(train_x, train_y, epochs = 2000,
                 batch_size=352, validation_split=0.12, verbose=1)

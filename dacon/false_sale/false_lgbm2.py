...
train = pd.concat(
    [train.drop(columns=one_hot_cols), pd.DataFrame(train_encoded, index=train.index)],
    axis=1,
)
test = pd.concat(
    [test.drop(columns=one_hot_cols), pd.DataFrame(test_encoded, index=test.index)],
    axis=1,
)
...
submission.to_csv("c:/data/dacon/false_sale/output/lgbm2_1.csv", index=False)

...
class TimeSeriesTransformer(nn.Module):
    def __init__(
        self, input_size, hidden_size, num_layers, num_heads, output_size, dropout
    ):
...
class AgriculturePriceDataset(Dataset):
    def __init__(self, dataframe, window_size=9, prediction_length=3, is_test=False):
        ...
        self.price_column = [
            ...
            if "평균가격(원)" in col and len(col.split("_")) == 1
        ][0]
        self.numeric_columns = self.data.select_dtypes(
            include=[np.number]
        ).columns.tolist()
...
def train_and_predict(품목_리스트):
    ...
    for 품목명 in pbar_outer:
        ...
        train_loader = DataLoader(
            train_data, CFG.batch_size, shuffle=True, num_workers=4
        )
        ...
        품목_predictions = []
        pbar_inner = tqdm(
            range(25), desc="테스트 파일 추론 중", position=1, leave=False
        )

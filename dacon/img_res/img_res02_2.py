...
def get_input_image(image, min_polygon_bbox_size=50):
    ...
    while True:
        ...
        if (bbox_x2 - bbox_x1) < min_polygon_bbox_size or (
            bbox_y2 - bbox_y1
        ) < min_polygon_bbox_size:
            continue
        ...
        polygon_func = random.choice(
            [random_polygon, random_star_shaped_polygon, random_convex_polygon]
        )
...
def get_ssim_score(true, pred):
    ...
    ssim_value = ski_ssim(
        true, pred, channel_axis=-1, data_range=pred.max() - pred.min()
    )
    return ssim_value
...
for idx, row in tqdm(train_df.iterrows(), total=len(train_df)):
    ...
    save_image_name = (
        os.path.basename(img_path).replace("TRAIN", "VALID").replace("png", "npy")
    )
...
for fold_idx, (train_indices, valid_indices) in enumerate(
    kf.split(train_df["image"], train_df["label"])
):
    ...
    valid_fold_df["image"] = valid_fold_df["image"].apply(
        lambda x: x.replace("TRAIN", "VALID").replace("png", "npy")
    )
    ...
    def __call__(self, examples):
        if self.mode == "train" or self.mode == "valid":
            ...
            for example in examples:
                ...
                images_gray_masked.append(
                    self.normalize_image(example["image_gray_masked"])
                )
                images_gt.append(self.normalize_image(np.array(example["image_gt"])))

            return {
                "masks": torch.from_numpy(np.stack(masks)).long(),
                "images_gray": torch.from_numpy(np.stack(images_gray))
                .unsqueeze(1)
                .float(),
                "images_gray_masked": torch.from_numpy(np.stack(images_gray_masked))
                .unsqueeze(1)
                .float(),
                "images_gt": torch.from_numpy(np.stack(images_gt))
                .permute(0, 3, 1, 2)
                .float(),
            }
...
class LitIRModel(L.LightningModule):
    def __init__(
        ...
        model_1,
        model_2,
        image_mean=IMAGE_PREPROC_MEAN,
        image_std=IMAGE_PREPROC_STD,
    ):
        super().__init__()
    ...
    def unnormalize(self, output, round=False):
        image_restored = ((output * self.image_std + self.image_mean) * 255).clamp(
            0, 255
        )
        if round:
    ...
    def training_step(self, batch, batch_idx):
        masks, images_gray_masked, images_gray, images_gt = (
            batch["masks"],
            batch["images_gray_masked"],
            batch["images_gray"],
            batch["images_gt"],
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        images_gray_restored, images_restored = self(images_gray_masked)
        images_gt, images_restored = self.unnormalize(
            images_gt, round=True
        ), self.unnormalize(images_restored, round=True)
        ...
        masked_ssim_score = 0
        hist_sim_score = 0
        for image_gt_np, image_restored_np, mask_np in zip(
            images_gt_np, images_restored_np, masks_np
        ):
            total_ssim_score += get_ssim_score(image_gt_np, image_restored_np) / len(
                images_gt
            )
            masked_ssim_score += get_masked_ssim_score(
                image_gt_np, image_restored_np, mask_np
            ) / len(images_gt)
            hist_sim_score += get_histogram_similarity(
                image_gt_np, image_restored_np, cv2.COLOR_RGB2HSV
            ) / len(images_gt)
        ...
        self.log(
            f"val_total_ssim_score", total_ssim_score, on_step=False, on_epoch=True
        )
        self.log(
            f"val_masked_ssim_score", masked_ssim_score, on_step=False, on_epoch=True
        )

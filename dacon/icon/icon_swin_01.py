...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = row.drop(["ID", "label"]).values.astype(np.uint8).reshape(32, 32)  # 32x32 이미지 복원
        label = row["label"]
...
def train_model(model, train_loader, val_loader, criterion, optimizer, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        for images, labels in train_loader:
            images, labels = images.to(device), labels.to(device).long()  # labels를 long 타입으로 변환
...
train_losses,val_losses, train_accs, val_accs = train_model(model, train_loader, val_loader, criterion, optimizer, epochs=100)
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        row = self.dataframe.iloc[idx]
        image = row.drop(["ID"], errors="ignore").values.astype(np.uint8).reshape(32, 32)  # 32x32 이미지 복원
...
output_df.to_csv("c:/data/dacon/icon/output/swin_transformer_01.csv", index=False)

...
for fold_idx, (train_index, val_index) in enumerate(
    skf.split(train_df, train_df["class"])
):
    ...
    train_dataloader = DataLoader(
        train_dataset, collate_fn=train_collate_fn, batch_size=BATCH_SIZE
    )
    val_dataloader = DataLoader(
        val_dataset, collate_fn=val_collate_fn, batch_size=BATCH_SIZE * 2
    )
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename=f"swinv2-large-resize-fold_idx={fold_idx}"
        + "-{epoch:02d}-{train_loss:.4f}-{val_score:.4f}",
        save_top_k=1,
...
for checkpoint_path in glob(
    "/home/aia/Study/dacon/bird/checkpoints/swinv2-large-resize*.ckpt"
):
    model = Swinv2Model.from_pretrained(
        "microsoft/swinv2-large-patch4-window12to16-192to256-22kto1k-ft"
    )

...
def predict_with_sam(image_path):
    # SAM 모델 로드
    sam_model = keras_cv.models.SegmentAnythingModel.from_preset("sam_base_sa1b")

    # 이미지 로드 및 전처리
    image = get_img_arr(image_path)  # 기존에 정의된 get_img_arr 함수를 사용하여 이미지를 로드
    image = np.expand_dims(image, axis=0)  # 모델 입력에 맞게 차원 확장

    # 점과 라벨 설정 (예시)
    points = np.array([[[512., 512.], [100., 100.]]])  # 이미지 내 점 위치
    labels = np.array([[1., 0.]])  # 1: 전경, 0: 배경
    box = np.array([[[[384., 384.], [640., 640.]]]])  # 영역 상자
    input_mask = np.ones((1, 1, 256, 256, 1))  # 입력 마스크
    ...
    # SAM 모델로 예측 수행
    outputs = sam_model.predict(inputs)
